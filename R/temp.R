...
#' #' #' out <- SANple::sample_fiSAN(nrep = 500, burn = 200, 
#' #' #'                     y = y, group = g, 
#' #' #'                     nclus_start = 2,
#' #' #'                     maxK = 20, maxL = 20,
#' #' #'                     beta = 1)
...
#'   
#'   
#'   max_DC <- max(estimated_dc)
#'   if(palette_brewed){
#'     colpal <- rev(grDevices::colorRampPalette(RColorBrewer::brewer.pal(11, 
#'                                                                        "Spectral"))(max_DC))
#'   }else{
#'     colpal <- 1:max_DC
#'   }
...
#'        xlim = range(x$params$y), main = "eCDFs colored by DC",
#'        xlab = "y", ylab = "eCDF",)
#'   for(j in 2:length(ecdfs)) { plot(ecdfs[[j]], verticals=TRUE, 
#'                                    do.points=FALSE, 
#'                                    add=TRUE, col = colpal[DCs[j]]) }
#'   
#'   graphics::boxplot(
#'     x$params$y ~ x$params$group,
#'     col = scales::alpha(colpal[estimated_dc], .7),
