...
// [[Rcpp::export]]
Rcpp::List sample_ficam_burn(int nrep, // number of replications of the Gibbs sampler
                             int burn,
...
)
{
  ...
  for(int iter = 0; iter < nrep ; iter++)
  {
    ...
    current_omega = dirichlet_sample_obs_weights(current_M,
                ...
                current_beta,
                current_maxK, 
                current_L,  // checked: added iter + 1
                maxK, maxL) ;
    ...
    current_L = fcam_sample_K(maxL -1,  // checked: added - 1
                              current_Lplus,
          1, 4, 3,
          current_beta,
          current_M) ;
    ...
    current_mu(arma::span(current_L, maxL-1)) = // span e' necessario?
         arma::zeros(maxL - current_L) ;
